...
function injectGlobalApisPanel() {
  ...
  card.innerHTML = `
    <div class="card-header">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="card-header-icon"><circle cx="12" cy="12" r="10"></circle><line x1="2" y1="12" x2="22" y2="12"></line><path d="M12 2a15.3 15.3 0 0 1 0 20"></path><path d="M12 2a15.3 15.3 0 0 0 0 20"></path></svg>
      <h2>APIs Globales</h2>
    </div>
    <div class="api-grid">
      <section class="api-tool">
        <h3>Pais</h3>
        <p>Consulta bandera, moneda e idioma con REST Countries.</p>
        <div class="api-input-row"><input type="text" id="countryInput" placeholder="Ej. Ecuador"><button type="button" id="btnCountryApi" class="btn-api">Buscar</button></div>
        <div id="countryResult" class="api-result">Sin consulta.</div>
      </section>
      <section class="api-tool">
        <h3>Clima</h3>
        <p>Consulta temperatura, humedad y descripcion con OpenWeather.</p>
        <div class="api-input-row"><input type="text" id="weatherCityInput" placeholder="Ej. Quito"><button type="button" id="btnWeatherApi" class="btn-api">Ver clima</button></div>
        <div id="weatherResult" class="api-result">Configura WEATHER_API_KEY para consultar.</div>
      </section>
      <section class="api-tool">
        <h3>Moneda</h3>
        <p>Convierte el total de USD a otra moneda con ExchangeRate API.</p>
        <div class="api-input-row"><input type="text" id="currencyInput" placeholder="EUR, COP, MXN" maxlength="3"><button type="button" id="btnExchangeApi" class="btn-api">Convertir</button></div>
        <div id="exchangeResult" class="api-result">Ingresa una moneda para convertir.</div>
      </section>
    </div>`;
...
async function fetchCountryInfo() {
  ...
  if (!query) {
    showToast('Escribe un pais para consultar.', 'error');
    return;
  }

  setApiResult('countryResult', 'Consultando pais...', 'loading');
  try {
    const response = await fetch(`https://restcountries.com/v3.1/name/${encodeURIComponent(query)}?fields=name,flags,currencies,languages`);
    ...
    const currencies = Object.entries(country.currencies || {})
      .map(([code, data]) => `${code} - ${data.name || 'Moneda'}`)
      .join(', ') || 'No disponible';
    const languages = Object.values(country.languages || {}).join(', ') || 'No disponible';
    ...
    setApiResult('countryResult', `<div class="country-result">${currentCountryInfo.flag ? `<img src="${currentCountryInfo.flag}" alt="Bandera de ${escapeHTML(currentCountryInfo.name)}">` : ''}<div><strong>${escapeHTML(currentCountryInfo.name)}</strong><span>Moneda: ${escapeHTML(currentCountryInfo.currencies)}</span><span>Idioma: ${escapeHTML(currentCountryInfo.languages)}</span></div></div>`, 'success');
...
async function fetchWeatherInfo() {
  ...
  if (!city) {
    showToast('Escribe una ciudad para consultar el clima.', 'error');
    return;
  }
  if (!WEATHER_API_KEY || WEATHER_API_KEY === 'TU_API_KEY_AQUI') {
    ...
    showToast('Falta configurar WEATHER_API_KEY.', 'error');
    return;
  }
...
async function convertTotalCurrency() {
  ...
  if (!currency || currency.length !== 3) {
    showToast('Escribe un codigo de moneda de 3 letras, por ejemplo EUR.', 'error');
    return;
  }
...
function handleAuthSubmit(event) {
  ...
  if (authMode === 'register') {
    ...
    if (users.some((user) => user.email === email)) {
      setAuthMode('login');
      return showToast('Ese correo ya esta registrado. Inicia sesion.', 'error');
    }
...
async function handleBudgetFormSubmit(event) {
  ...
  const record = {
    ...
    exchangeInfo: currentExchangeInfo,
...
async function generatePDF(item) {
  ...
  if (logoBase64) doc.addImage(logoBase64, 'PNG', 12, 8, 96, 22);
  else drawPdfLogo(doc);
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);
  doc.text('Descripcion', 25, startY + 8);
  ...
  if (item.countryInfo || item.weatherInfo || item.exchangeInfo) {
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(12);
    doc.text('Datos de APIs integradas', 18, y);
    y += 10;
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(10);
    doc.setTextColor(51, 65, 85);
    if (item.countryInfo) {
      doc.text(`Pais: ${item.countryInfo.name} | Moneda: ${item.countryInfo.currencies}`, 25, y);
      y += 7;
      doc.text(`Idioma: ${item.countryInfo.languages}`, 25, y);
      y += 8;
    }
    if (item.weatherInfo) {
      doc.text(`Clima en ${item.weatherInfo.city}: ${item.weatherInfo.temperature} C, humedad ${item.weatherInfo.humidity}%, ${item.weatherInfo.description}`, 25, y);
      y += 8;
    }
    if (item.exchangeInfo) {
      doc.text(`Conversion: ${formatCurrency(item.exchangeInfo.totalUSD)} USD = ${item.exchangeInfo.convertedTotal} ${item.exchangeInfo.currency}`, 25, y);
      y += 8;
    }
  }
  ...
  doc.text('Cotizacion generada por CotizaAPI Global con REST Countries, OpenWeather y ExchangeRate API.', 18, pageHeight - 24);
...
function init() {
  ...
  injectGlobalApiStyles();
  injectGlobalApisPanel();
  loadLocalMirror();
  bindGlobalApiEvents();
  setAuthMode('login');
  ...
  renderHistory();
  updatePreviewDisplay();
  console.log('CotizaAPI Global iniciado correctamente.');
}
